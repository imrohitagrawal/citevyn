...
| E2E live-mode (Playwright, 5)     | 🟢 green | CI job `Live-mode Playwright (stub backend)` — **a required check** as of the 2026-09-16 branch-protection reading (#379's objective; this row said "advisory, not a required check" until then). It is the only job that executes the five live-only tests. See `docs/TEST_STRATEGY.md` §12.1 |
| Visual snapshots (22)             | 🟡 advisory, baselines pending | CI job `Visual snapshots (Linux baselines, advisory)` (#325) compares against `*-chromium-linux.png` inside `mcr.microsoft.com/playwright`; locally `npm run test:visual` uses the `-darwin` set. **The `-linux` baselines are not committed yet** (#325-PENDING-BASELINES), so the job is red until they land — it is advisory, so that blocks nothing. See `docs/TEST_STRATEGY.md` §12.2 |
