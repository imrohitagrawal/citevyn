...
| E2E demo-mode (Playwright, 230)   | 🟢 green | CI job `Demo-mode Playwright (no visual snapshots)`; locally `npm run test:ui`. The count is the whole default suite minus its 22 visual tests, and the job asserts that differential rather than the number — re-measure with `npx playwright test -c playwright.demo-ci.config.ts --list` before quoting it (it read **180** here until 2026-09-16) |
| E2E live-mode (Playwright, 5)     | 🟢 green | CI job `Live-mode Playwright (stub backend)` — **a required check**, measured 2026-09-16; #379's objective, and this row claimed the opposite until then. It is the only job that executes the five live-only tests. See `docs/TEST_STRATEGY.md` §12.1 |
| Visual snapshots (22)             | 🟡 advisory | CI job `Visual snapshots (Linux baselines, advisory)` (#325) compares against `*-chromium-linux.png` inside `mcr.microsoft.com/playwright`; locally `npm run test:visual` uses the `-darwin` set. The `-linux` baselines are committed. The job stays **advisory** deliberately: promoting it is a branch-protection change and therefore owner-only, and a font-rendering shift should prompt someone to look at a diff image rather than block every merge. See `docs/TEST_STRATEGY.md` §12.2 |
