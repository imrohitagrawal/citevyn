...
| Visual snapshots (22)             | 🟡 advisory, baselines pending | CI job `Visual snapshots (Linux baselines, advisory)` (#325) compares against `*-chromium-linux.png` inside `mcr.microsoft.com/playwright`; locally `npm run test:visual` uses the `-darwin` set. **The `-linux` baselines are not committed yet**, so the job is red until they land — it is advisory, so that blocks nothing. See `docs/TEST_STRATEGY.md` §12.2 |
