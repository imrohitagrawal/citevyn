 ...
 *
 * `contrastRatio(colour.slice(0, 3), bg)` — what this file did before — scores
 * the un-premultiplied colour as if it were opaque. `.cta-footnote` in this
 * very stylesheet is `color-mix(in srgb, var(--bg) 50%, transparent)`, so the
 * shape is live, not hypothetical: at 50% alpha the opaque reading is roughly
 * double the ratio a reader actually gets.
 */
function paintedRatio(colour: readonly number[], backdrop: readonly number[]): number {
  const a = colour[3] ?? 1;
 ...
 * after `enterChat`.
 */
async function installKit(page: import("@playwright/test").Page) {
  await page.addInitScript(() => {
     ...
     * down to 1.47:1. An earlier draft's docstring said this function reported
     * "a `::before` overlay" while the body never passed a pseudo argument to
     * `getComputedStyle` at all; `focus-ring.spec.ts` learned the same lesson
     * the hard way.
     *
    ...
    const opaqueBlockers = (selector: string, index: number): string[] => {
      ...
      for (let n: Element | null = el; n; n = n.parentElement) {
        ...
        const name = `${n.tagName.toLowerCase()}${n.className && typeof n.className === "string" ? "." + n.className.trim().split(/\s+/).join(".") : ""}`;
        ...
        for (const pseudo of ["::before", "::after"]) {
          ...
          if (paints) {
            bad.push(
              `${name}${pseudo} content:${ps.content} background:${ps.backgroundImage !== "none" ? ps.backgroundImage : ps.backgroundColor}`,
            );
          }
        }
      }
