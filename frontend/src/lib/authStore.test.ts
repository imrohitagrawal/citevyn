...
describe("bootstrapAuth", () => {
  ...
  it("resolves to signed-in for a registered user", async () => {
    const user = { request_id: "req_1", user_id: "usr_a", email: "a@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false };
...
describe("login/register", () => {
  it("login sets signed-in state", async () => {
    const user = { request_id: "req_1", user_id: "usr_b", email: "b@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false };
    vi.mocked(fetch).mockResolvedValueOnce(jsonResponse(user));
  ...
  it("register sets signed-in state", async () => {
    const user = { request_id: "req_1", user_id: "usr_c", email: "c@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false };
...
describe("logout", () => {
  it("drops to anonymous even if the network call fails", async () => {
    __testOnly.setState({ status: "signed-in", user: { request_id: "req_1", user_id: "usr_a", email: "a@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false } });
...
describe("subscribeAuth", () => {
  it("notifies listeners on state change", async () => {
    ...
    const user = { request_id: "req_1", user_id: "usr_d", email: "d@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false };
    vi.mocked(fetch).mockResolvedValueOnce(jsonResponse(user));
...
describe("401 interceptor", () => {
  it("drops signed-in state to anonymous when ANY apiFetch call 401s", async () => {
    __testOnly.setState({
      status: "signed-in",
      user: { request_id: "req_1", user_id: "usr_a", email: "a@example.com", anonymous: false, providers: [], has_password: true, password_step_up: false },
    });
