/**
 * The below-the-fold marketing strip (#358) — a separate MODULE so rollup emits
 ...
 * Both wrappers come from this one file, so the reader still pays exactly ONE
...
/**
 * The deferred sections ABOVE the eager InteractiveDemo. Default export so
 * `lazy(() => import("./landing-strip"))` reads like the app's four existing
 * lazy surfaces (AuthModal, HistoryDrawer, ConnectedAccountsDrawer, Nudge).
 */
export default function LandingStripTop({ onAsk }: { onAsk: (q: string) => void }) {
  return (
    <>
      <Personas onAsk={onAsk} />
      <HowItWorks />
      <WhyDifferent />
    </>
  );
...
/** The deferred sections BELOW the eager InteractiveDemo. Same module, same chunk. */
export function LandingStripBottom({
  onGetPro,
  onOpenChat,
  openFaq,
  toggleFaq,
  ...
  return (
    <>
      <Pricing onGetPro={onGetPro} onOpenChat={onOpenChat} />
      <FAQ openFaq={openFaq} toggleFaq={toggleFaq} />
      <CTABanner onOpenChat={onOpenChat} />
      <Footer />
    </>
  );
}
