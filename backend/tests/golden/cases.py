...
class GoldenCase:
    ...
    expect_search_hit_product_area: str | None
    expect_last_domain: str | None
    expect_last_answer_contains: tuple[str, ...]
    expect_last_citation_url_contains: str | None
    expect_answer_contains: tuple[str, ...]
    expect_citation_url_contains: str | None
    expect_answer_cites_index: bool | None
    raw: dict[str, Any]
    path: pathlib.Path
    ...
    def from_dict(cls, d: dict[str, Any], path: pathlib.Path) -> GoldenCase:
        if "id" not in d or "type" not in d or "area" not in d:
            raise ValueError(f"{path}: missing required keys id/type/area; got keys {sorted(d)}")
        return cls(
            id=d["id"],
            ...
            expect_search_hit_product_area=d.get("expect_search_hit_product_area"),
            expect_last_domain=d.get("expect_last_domain"),
            expect_last_answer_contains=tuple(d.get("expect_last_answer_contains", ())),
            expect_last_citation_url_contains=d.get("expect_last_citation_url_contains"),
            expect_answer_contains=tuple(d.get("expect_answer_contains", ())),
            expect_citation_url_contains=d.get("expect_citation_url_contains"),
            expect_answer_cites_index=d.get("expect_answer_cites_index"),
            raw=d,
            path=path,
        )
...
def filter_cases(cases: Iterable[GoldenCase], *, ids: list[str] | None = None) -> list[GoldenCase]:
