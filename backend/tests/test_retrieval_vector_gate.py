...
def _doc(area: str):
    ...
    # Same for ``last_fetched_at`` (NOT NULL on the real row), read for the
    # "docs as of" stamp (ADR-0005 §6).
    return SimpleNamespace(
        source_name=area,
        title=f"{area} doc",
...
async def test_the_vector_arm_carries_each_documents_fetch_date() -> None:
    ...
    builds hits. RED if ``VectorRetriever`` stops setting ``fetched_at``."""
    rows = _rows([("codex", 0.60), ("claude_api", 0.30)])
    vr = VectorRetriever(_mock_session(rows), embedder=_FakeEmbedder(), global_confidence=None)
    ...
    assert hits, "precondition: the mocked dialect let the arm actually build hits"
    assert {h.fetched_at for h in hits} == {datetime(2026, 9, 1, tzinfo=UTC)}
