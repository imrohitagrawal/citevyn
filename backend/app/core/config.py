...
class Settings(BaseSettings):
    ...
    # about half this many SHA-256 hashes; 50,000 is well under a second), and
    # how long a challenge stays valid. Only used with the access model on.
    bot_check_secret: str | None = None
    bot_check_max_number: int = Field(default=50_000, ge=1)
    ...
    def _require_bot_check_secret_when_the_access_model_is_on_in_production(self) -> "Settings":
        # Without it every sign-up would be refused (the check fails closed).
        if (
            self.environment == "production"
            and self.access_model_enabled
            and not self.bot_check_secret
        ):
            raise ValueError(
                "CITEVYN_ACCESS_MODEL_ENABLED is on in production but "
                "CITEVYN_BOT_CHECK_SECRET is not set: every sign-up would be refused."
            )
        return self
