...
| [#197](https://github.com/imrohitagrawal/citevyn/issues/197) | **CLOSED — won't build.** #153 Layer 4 ("persist the per-user rate limiter") is a no-op that would make things worse. Two verified reasons: (1) **it already exists** — `infra/docker/.env:104` sets `CITEVYN_REDIS_URL`, so `_build_limiter` (`backend/app/core/rate_limit.py:338`) already selects `RedisRateLimiter`; the prod bucket is a Redis `ZSET` and already outlives an api restart. (2) **The bucket is global, not per-user** — `require_demo_api_key` returns the constant `DEMO_USER_ID` (`backend/app/core/security.py:29`) and `_bucket_key` keys on it (`rate_limit.py:257`), so all demo visitors share ONE bucket. Making that bucket *more* durable is a global-lockout amplifier, not a spend control. Layer 3 (§9 daily budget, PR #188) is the real, restart-proof control. **The actual missing piece is per-session identity, not persistence** — branch `feat/153e-persist-rate-limiter` is retained for if that ever lands. |
