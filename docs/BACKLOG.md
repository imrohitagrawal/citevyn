...
| [#84](https://github.com/imrohitagrawal/citevyn/issues/84) | CiteVyn-meta maturation: intent-detect token-absent phrasings, real-embedder no_answer golden, golden-in-CI, offline-copy convergence, refusal-copy nudge, `/about` deploy. **Now carries a concrete instance:** the speech-to-text garbles "sitewin" / "site win" miss `\bcitevyn\b` and refuse | backend / frontend | Low (the sitewin instance hits the owner's own demo flow) | #49 / PR #83 review; #169 live verification |
...
| [#170](https://github.com/imrohitagrawal/citevyn/issues/170) | Corpus: `claude_code.md` has no installation content, so "How do I install Claude Code?" refuses (identical single-turn and as a follow-up — a corpus gap, not retrieval) | corpus / worker | Medium (an obvious first question for a CLI tool, and a likely demo ask) | #169 live verification |
