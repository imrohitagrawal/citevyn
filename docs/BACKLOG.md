...
| [#237](https://github.com/imrohitagrawal/citevyn/issues/237) | **`_CITATION_RE` counts `[n]` inside code spans / fenced blocks as citations.** `findall("Use \`arr[2]\` here [1].")` → `['2','1']`, so a phantom card is attached and `confidence` (= `len(cited)/len(evidence)`) is skewed. Pre-existing and live; #236 makes the phantom card carry a visible number. **Deliberately NOT fixed alongside #215**: stripping code spans before `findall` turns an answer whose only citation sits inside a fence from `['1']` into `[]`, and an empty `cited_indices` is an unconditional refusal **plus** an `exact_lookup` retry (a second, paid LLM call) — a new refusal path in the PR whose purpose was removing one, covering only 2 of 4 fence forms. Mandatory shape if picked up: `findall(strip(t)) or findall(t)`, never letting the strip empty the set | backend / answer | Medium — ~~Low (one mislabelled card; the B3 chip plugin skips `code`/`inlineCode` so it can never produce a wrong chip)~~ **that rationale is void: the B3 chip plugin DOES NOT EXIST on `main`.** `citation-chip` appears only as hardcoded marketing copy (`landing-sections.tsx:297`) and CSS; answer text renders raw at `ChatView.tsx:176`. Severity also rose with #215: contiguity used to *accidentally* mask a subset of this — an answer citing only `[1]` whose code block contains `delays[3]` was previously REFUSED, and is now served with an authoritative source card numbered **3** that visually matches the subscript the reader can see | #215 plan attack; rationale corrected during #215 review |
