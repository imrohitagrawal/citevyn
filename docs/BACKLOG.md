...
| [#229](https://github.com/imrohitagrawal/citevyn/issues/229) | **`/health/index` and `GET /v1/admin/index_versions` report `evaluation_run_id: null` even when the index HAS passing evaluation evidence.** `index_versions.evaluation_run_id` is declared in the model, migration `0001` and the admin schema but **nothing has ever assigned it**; the promotion gate resolves evidence the other way round, via `evaluation_runs.index_version`. Verified live: production `v1` measured `pass_rate=1.0` and the gate reports `promotable WITHOUT force: True`, while `/health/index` still shows `null`. An operator following RUNBOOK §3.7 / DEPLOY_FLY §4.3 reads that as "no evidence" and reaches for `?force=true` — the exact muscle memory #216 removed. Fix `evaluate_index` to set it; keep `_latest_completed_run` authoritative for the gate | backend / ops | Medium | live verification of #216 |
