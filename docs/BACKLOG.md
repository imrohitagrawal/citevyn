...
| [#226](https://github.com/imrohitagrawal/citevyn/issues/226) | **Retrieval's Tier-3 provenance check reads the ACTIVE index's stamp, not the one being retrieved.** `HybridRetriever._active_index_stamp` resolves by `status == active`, ignoring `active_index_version`, so any caller retrieving against a NON-active index (exactly what the #216 promotion evaluator does) gets the vector arm ENABLED against a mismatched index — meaningless cosine, the silent failure #57 exists to prevent. Measured, not theorised. PR #225 closes it for the promotion path only; the retrieval fix changes the shared request path and interacts with the #58 dual-active guard (which today ENABLES the arm on unknown provenance), so it needs a deliberate decision | backend / retrieval | Medium | adversarial review of #216 |
