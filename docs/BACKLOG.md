...
| [#84](https://github.com/imrohitagrawal/citevyn/issues/84) | CiteVyn-meta maturation: ~~name recognition~~ (**item 1 done, PR #172** — speech-to-text aliases route + canonicalize; two-word `"site win"` forms are a deliberate tested MISS, they need an intent classifier not a regex), real-embedder no_answer golden, golden-in-CI, offline-copy convergence (frontend `matchCitevynMeta` knows no aliases), refusal-copy nudge, `/about` deploy | backend / frontend | Low | #49 / PR #83 review |
