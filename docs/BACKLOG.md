...
| Issue | Title | Area | Notes |
|---|---|---|---|
| [#270](https://github.com/imrohitagrawal/citevyn/issues/270) | Real user accounts (login): ADR-0004 implementation, PRs 0-12 | backend / frontend / auth | Owner-approved (`~/.claude/plans/i-also-want-you-hashed-marble.md`), overriding the doless recommendation. **PR 0, PR 1, PR 2 (partial), PR 3, and PR 4 DONE — the full ultracode "no further than PR 4 without check-in" allowance used.** PR 1: `_get_session_or_404`/`_require_session` now filter on `user_id` + unexpired; mismatch is 404 never 403; mutation-tested. PR 2: security-headers middleware on every response incl. the StaticFiles mount, `/docs`+`/redoc`+`/openapi.json` disabled in production — health-route-behind-admin deferred to **[#273](https://github.com/imrohitagrawal/citevyn/issues/273)**. PR 3: migration `0007` `auth_sessions` + `app.core.auth_sessions.resolve_principal` — every session/message route now resolves a real per-visitor principal (anonymous `anon_<uuid4hex>`, cookie `<id>.<secret>`, only `sha256(secret)` stored) instead of the constant `demo_user`, so PR 1's ownership predicate now actually differentiates visitors; `CORS allow_credentials=False` pinned by test. PR 4: `app.core.passwords` (Argon2id, OWASP params, `asyncio.Semaphore(2)` concurrency bound, dummy-hash timing-parity verify) — no routes yet, new dep `argon2-cffi`. All four security-reviewed by subagent, no findings; mutation-tested. **Next action for a future session:** PR 5 (migration `0008`, `users` identity columns, `sessions` FK RESTRICT->CASCADE) is a one-way door — verify in prod before PR 6 ships, and get an explicit go-ahead before continuing past PR 4, per the brief. See `docs/ADR/0004-user-accounts.md` |
