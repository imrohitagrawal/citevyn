...
| [#210](https://github.com/imrohitagrawal/citevyn/issues/210) | **Index promotion has NO quality gate.** `CITEVYN_INDEX_PROMOTION_MIN_PASS_RATE` is declared in `config.py` and read by nothing; `promote_version` promotes unconditionally. `RELEASE_PLAN` §7 lists promotion gates as though they are enforced. Latent since Slice 8; surfaced when a deploy runbook promised operators the guarantee. The operator running `make golden` is currently the gate | backend / ops | Medium (promotion is when bad retrieval reaches users) | T3 review of the Fly deploy branch |
