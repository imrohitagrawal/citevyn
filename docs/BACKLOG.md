...
| [#231](https://github.com/imrohitagrawal/citevyn/issues/231) | **CI tests the frontend on Node 20; production ships a bundle built on Node 22 — nothing reconciles them.** `frontend.yml:36`/`frontend-live-e2e.yml:28` pin `setup-node` to 20, `Dockerfile.api:55` builds the SHIPPED bundle on 22, and there is no `.nvmrc` or `engines` field. So CI has never validated the artifact production serves. Dependabot **PR #227** (node 22→26) would widen that to six majors while CI stays on 20 and reports green. Fix: `.nvmrc` as single source of truth + `node-version-file:` in both workflows + Dockerfile aligned + a guard test (the repo already has that pattern); reconcile on 22 FIRST, then bump CI and Dockerfile together as one deliberate change. **Do not merge #227 alone** | frontend / ci | Medium | sequencing PR #227 |
