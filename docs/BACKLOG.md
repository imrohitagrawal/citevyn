...
| [#70](https://github.com/imrohitagrawal/citevyn/issues/70) | Cache: an answer degraded by a transient EmbedderUnavailable (Tier-1) is still cached to TTL | cache / observability | Low — **fix in flight** (runtime vector-degrade signal from `retrieve()` gates the cache write; shared fix with #72) | #65 / PR #69, ADR-0003 |
| [#72](https://github.com/imrohitagrawal/citevyn/issues/72) | Cache: exact-lookup answers are needlessly skipped from the cache under an embedder mismatch | cache | Low — **fix in flight** (same runtime-degrade-signal fix as #70) | #65 / PR #69 |
