...
| `--faint` | `#9a978f` | **Not usable for text, and consumed by nothing (#396).** 2.77:1 on `--bg`, 2.92:1 on `--surface`, 2.58:1 on `--surface-2` — the best of those is under even WCAG's relaxed 3:1 large-text floor, so it clears no text threshold at any size or weight. Kept declared; retiring a token is an owner decision |
...
Theme-SPLIT as of #403. Before it, each of these was one value used on both
a near-white and a near-black surface, and these nine selectors shipped below
the WCAG AA 4.5:1 floor as a result: `.refusal-badge`, `.compare-footer.good`,
`.compare-footer.bad`, `.source-badge.zero`, `.check-icon`, `.hero-nudge`,
`#hero-input::placeholder`, `.notice-rate-limit`, `.notice-error`.

No count is given for "runs", because the number depends on what you count:
nine selectors, ten DOM runs (`.check-icon` renders twice), eleven failing
selector x theme measurements, twelve failing DOM-run x theme measurements. The EXECUTABLE list is `RETUNED` in
`frontend/tests/contrast-floor.spec.ts`, which is what re-measures them — add a
selector there and here together, and quote that list rather than a number.
Earlier drafts of this section said "seven" and the #403 changelog row below
said "eleven"; both were wrong, and neither matched the other. The Dark column was ALSO wrong before #403,
in a second way this table did not disclose: it listed `#4ade80` / `#f87171` /
`#fbbf24`, which are the `[data-style="devtools-alt"]` skin's palette, and
nothing in the app sets `data-style`, so those values rendered for nobody.
What shipped in dark mode was the light column.

| Purpose | Token | Light | Dark |
|---------|-------|-------|------|
| Success | `--color-success` | `#157a4a` | `#2fae72` |
| Error (invented) | `--color-error` | `#a84437` | `#e08b7d` |
| Error, deeper (0 SOURCES chip, ✗ footer) | `--color-error-strong` | `#8a3324` | `#f5ab9a` |
| Refusal amber | `--color-warning` | `#8a5418` on `--hl-soft` | `#e0a458` on `--hl-soft` |

`--refusal-amber` is an ALIAS of `--color-warning` (`var(--color-warning)`
declared once on `:root`), not a fifth value. Light takes the darker member of
each pair and dark the lighter one; `--color-error-strong` therefore stays the
DEEPER of the two reds in light and the BRIGHTER of the two in dark, because
that is what "stronger" means once the page inverts.

`--color-info` (`#0284c7`) is declared and consumed by nothing.
...
Two facts here were stale and are corrected against `landing.css:724-734`: the
size was written as `12px` when the shipped rule is `11.5px`, and the colour was
`--faint`, which at that size needs 4.5:1 and measures 2.77:1 light / 3.49:1
dark (#396). The `font-weight` and `letter-spacing` lines above still differ
from the shipped rule and are left as the design intent, not corrected here —
`landing.css` is authoritative for what renders.
...
`background: color-mix(in srgb, var(--bg) 94%, transparent)`. Height: `64px`.

The fill was `82%` until #403. `.cta-banner` (`background: var(--ink)`) scrolls
under this bar, and at 82% the nav links measured 3.64:1 light / 4.20:1 dark
over the `0.82 x --bg + 0.18 x --ink` composite — under the 4.5:1 that 14px text
owes. At `94%` the worst case across all five surfaces the page can put behind
the bar is 4.69:1 light / 6.08:1 dark. `frontend/tests/contrast-floor.spec.ts`
composites this exact alpha over each of those surfaces, so the number cannot
drift back without going red; change it here and there together.
...
4. **Input box:** Rounded 12px, border `1px solid var(--border-2)`,
   contains:
   - `▸` glyph in `--muted` mono — an INLINE style in `Hero.tsx`, not a rule,
     which is why every CSS-only sweep of `--faint` missed it until #396
   - Text input (flex: 1)
   ...
   - Shake on empty submit: `animation: cv-shake 0.4s ease`,
     border color → `var(--color-warning)` (#403: `#8a5418` light,
     `#e0a458` dark — it used to be the theme-invariant `#b4732a`)
...
- Each card has:
  ...
  2. **Step number** (01/02/03) in `JetBrains Mono` 24px, weight 700,
     color `--muted` (#396). 24px/700 IS large text under WCAG, so 3:1 would
     have been enough — but the number sits in `.step-meta`, a SIBLING of the
     inset panel, so it paints on the card's `--surface` (#ffffff) rather than
     on the panel's `--surface-2`, and `--faint` there measures **2.92:1**,
     0.08 short of even that relaxed floor. Measured in Chromium, both themes
...
- "0 SOURCES" badge (red, `var(--color-error-strong)` — `#8a3324` light, `#f5ab9a` dark; `#b0503f` before #403)
...
- Subtext: a bare `<p>`, `color: var(--bg)` via
  `.cta-banner > p:not(.cta-footnote)`. **This line was wrong in two ways and
  is corrected against the stylesheet (#396).** It described a
  `color-mix(in srgb, var(--bg) 72%, transparent)` that exists nowhere in
  `landing.css` — the nearest real value is `.cta-footnote`'s **50%**, a
  different element — and the subtext had **no rule at all**, so it inherited
  `.cta-banner`'s `color: var(--ink)` onto `.cta-banner`'s
  `background: var(--ink)`. Measured in Chromium: rgb(28, 27, 25) on
  rgb(28, 27, 25) light and rgb(240, 239, 233) on rgb(240, 239, 233) dark —
  **1.00:1 in both themes**, a whole sentence invisible to every sighted
  visitor. With `--bg` it measures **16.35:1 light / 15.69:1 dark**, matching
  the `h2` beside it
...
- Mono `.demo-badge`: "LIVE — answering from the docs" in live mode,
  "DEMO — sample answers" otherwise. (This line read "DEMO — canned responses"
  until 2026-09-08. That was ACCURATE spec text that went stale, not an invented
  string: `ChatView.tsx` rendered exactly it from `2503dd4` (2026-07-09) until
  `9b689d5` (#138, 2026-07-17) replaced it without updating this file — two
  months of undetected drift, shown by `git log -S "DEMO — canned responses"
  -- frontend/src/components/ChatView.tsx`. Found while fixing #380 and corrected
  in the same change; the "never existed" wording that first accompanied it was
...
- **Waiting (live only):** a bot bubble holding three pulsing dots
  (`cv-pending`) and the label "Searching the docs…". It scrolls with the rest
  of the list. It is **not** a live region — that role was removed in #356 gap
  1 for the reasons the composer bullet below gives, and this paragraph
  described it as `role="status"` in an `aria-live="polite"` region for two
  commits after it stopped being one. The persistent region beside the composer
  is what tells a screen reader the question was accepted.
...
- Disclaimer `.composer-hint`: 11.5px, inheriting the body face
  (`"Geist", system-ui, sans-serif`) — it declares **no** `font-family` of its
  own; `.composer-prompt` is the mono one. (This line said `JetBrains Mono` 11px
  until 2026-09-08. Unlike the top-bar drift above, that pair never matched the
  code at all: `landing.css` has set `font-size: 11.5px` with no `font-family`
  since the file's first commit, `2503dd4`.) Two sentences. The first is always
  "CiteVyn answers from the official docs." The second states the MODE, and is
  keyed on `live` and on whether an **ANSWER is on screen** (#380) — the hint
  renders outside the empty/non-empty branch, so it is visible before any answer
  exists:
  - no answer on screen + live → "Answers here are generated live."
  - no answer on screen + demo → "Answers here are samples for the demo."
  - answer on screen + live → "This answer was generated live, just now."
  - answer on screen + demo → "This is a sample answer for the demo."

  "Answer on screen" is `messages.some((m) => !m.isUser && !m.errorKind)`, NOT
  `!chatEmpty`. `chatEmpty` is `messages.length === 0`, and the user's own bubble
  is appended BEFORE the request starts, so a non-empty transcript with no answer
  is the normal live state for the whole round trip — keying on emptiness left
  the sentence claiming an answer for exactly that window. An `errorKind` bubble
  is client-side transport copy, not a generated answer, so it does not count.
  The no-answer pair is deliberately GENERAL ("Answers here…") rather than
  about a specific answer, so it claims nothing about content that is not on
  screen. It is not dropped: a mode sentence keeps the live/demo signal at the
  moment the reader is about to type, and it lets the no-answer test assert a
  string that is PRESENT rather than only assert an absence.
- The hint is a plain `<p>`, deliberately **not** a live region. The persistent
  `role="status"` region beside it does all the announcing; an `aria-live` here
  would read a copy swap aloud that the reader never asked about.
...
- A question asked from a **landing entry point** while an answer is in flight
  ...
  - A draft already in the box is the reader's own and is never overwritten —
    ...
    overwritten is gone. A toast SAYS so — "Kept your draft" — so the drop is
    never silent. (This paragraph said the toast was "recorded in #356 instead
    of spent" against 66 B of remaining budget; it was restored in PR #359 once
    the ceiling was re-set deliberately, and the sentence was left behind.)
  ...
  - **The refusal itself is announced, not only the state** (#356 gap 2). The
    same region says "Not sent. CiteVyn is still answering your previous
    question. Your text is kept." It takes the region over rather than living in
    a second one: the copy is a superset of the in-flight sentence, so nothing
    is lost while it holds it.
  - **`refusedInFlight` is reducer state beside `pending`, not a latch in the
    view.** It is SET from the `inFlight` **ref** (never from the rendered
    `pending`, which is a render behind it by construction and would announce
    refusals that never happened) and CLEARED by the same `SET_PENDING(0)` that
    clears `pending`, in the same state object. That is the whole design: the
    two cannot disagree, so a stale refusal is unrepresentable. Two view-side
    shapes were tried first and a skeptic broke both — a latch cleared by an
    effect keyed on `pending` transitioning left a refusal masking the arrival
    announcement for the rest of the mount, and gating the render on `pending`
    only hid that latch, so the next genuine request announced "Not sent" about
    a question that **was** sent.
  - A mid-flight remount (landing → chat) **does** render the refusal, and that
    is correct: the flag is only ever set for a window that is still open, so
    the reader's text really is still unsent.
  - **Stated limitation:** a *second* refusal inside the *same* in-flight window
    ...
    window does announce, because the flag returns to false in between.
...
| 2026-09-08 | The composer hint's second sentence is keyed on whether an ANSWER is on screen (`messages.some((m) => !m.isUser && !m.errorKind)`), not on `live` alone and not on `chatEmpty`: a chat with no answer on it reads "Answers here are generated live." / "Answers here are samples for the demo." instead of "This answer was generated live, just now." Two stale §3 lines corrected in passing, and the font claim one of them carried | #380 — `.composer-hint` renders OUTSIDE the `chatEmpty ? … : …` branch (which opens at `ChatView.tsx:478` and closes at `:577`, while the composer starts at `:606` and the hint at `:675` — line numbers as of `a756847`, before this change; at `0551ba9` they are `:495`, `:594`, `:623` and `:692`), so the sentence was on screen before any answer existed and asserted one did. **The first attempt keyed it on `chatEmpty` and review measured it still false on the COMMON path.** `chatEmpty` is `messages.length === 0`, but `submitChat` appends the USER bubble and THEN awaits the network, so the whole live round trip is non-empty with no answer: measured end-to-end through the real component tree as `user-msgs=1 bot-msgs=0` with the hint reading "CiteVyn answers from the official docs. This answer was generated live, just now." — verbatim the reported symptom. Three reviewers converged on it independently. It is shipped-only: demo mode batches the user and bot bubbles into one React commit, so that window does not exist there and the demo-CI Playwright job cannot see it. The `!m.errorKind` half is the second finding — a rate-limit or network bubble is copy `handleApiError` builds on the client, not a generated answer, and was being counted as one. **Deliberately NOT the issue's suggested fix**, which was to drop the sentence when empty: a mode-stating sentence keeps the live/demo signal at the moment the reader is about to act, and it lets the no-answer test assert a string that is PRESENT rather than only assert an absence — a non-vacuous guard. The no-answer strings are GENERAL ("Answers here…") rather than about a specific answer precisely so they claim nothing about content that is not on screen. No test asserted this copy before, and `live: true` appeared in NO test in `ChatView.test.tsx` before this work (the first attempt claimed "ZERO vitest tests", which is false — `useLandingState.test.tsx:79` sets `mockIsLive.mockReturnValue(true)` in a `beforeEach`). There are now nine tests, each asserting the full rendered `textContent` with `toBe`, plus one asserting exactly ONE `.composer-hint` exists (`querySelector` returns the first match, so a second one would be invisible). The first attempt's two `not.toContain` lines are DELETED: each sat after a `toBe` on the same expression, making the containment a compile-time constant that could not fail. Non-vacuity now comes from holding BOTH sides of the predicate over the same non-empty transcript, so neither `hasAnswer` pinned false nor `!chatEmpty` satisfies the set. **Mutants, enumerated rather than counted** (`frontend/scripts/mutate-a11y-guards.sh`): collapse the no-answer branch onto the answer copy; key on `live` alone; invert the predicate; `hasAnswer` -> `!chatEmpty`; drop `&& !m.errorKind`; test `m.isUser` instead of `!m.isUser`; `messages.length > 0`; `hasAnswer && !pending`; invert `live` inside the no-answer branch; invert `live` inside the answer branch; drop the mode sentence; drop the first sentence; drop the `{" "}` separator; rename the class the tests select on; delete the whole `<p>` — fifteen, all killed; the harness reports KILLED: 38 SURVIVED: 0, all restored byte-identical. TWO §3 lines were stale at `a756847`, and between them they carried THREE wrong facts. Both quoted "DEMO — canned responses", which was ACCURATE spec text that went stale rather than an invented string: `ChatView.tsx` rendered exactly it from `2503dd4` (2026-07-09) until `9b689d5` (#138, 2026-07-17) replaced it with "DEMO — sample answers" — two months of undetected drift, shown by a pickaxe log over that string. The third wrong fact rode on the composer line beside the second quote, and the first attempt fixed the quote while carrying the font pair forward verbatim: `.composer-hint` was documented as `JetBrains Mono` 11px, and `landing.css` has set `font-size: 11.5px` with NO `font-family` (so it inherits `"Geist", system-ui, sans-serif` from `body`) since `2503dd4`. `.composer-prompt` is the mono one. **No behavior change beyond the copy predicate.** **+33 B gzip, measured** (62,974 before #380; 63,007 after; budget 64,479, headroom 1,505 -> 1,472). Both `chat-empty` visual baselines pass UNRECORDED on the final tree, re-run after this round (2 passed). That is a TOLERANCE pass, not an identity one, and an earlier draft of this row said otherwise: the baselines were last recorded at `1240484` (#336, 2026-09-04), so the stored pixels still read "This is a sample answer for the demo." while the empty chat now renders "Answers here are samples for the demo."; they pass because that one 11.5px line stays inside `maxDiffPixelRatio: 0.02`. Proven to bite: a deliberately long three-line hint FAILS both at ratio 0.04, 19,003 px light / 18,220 px dark. `visual.spec.ts` is `testIgnore`d by `playwright.demo-ci.config.ts`, so that was a local check, not a CI one |
| 2026-09-09 | Every one of the 26 `--faint` text uses moved to `--muted` — 24 rules in `landing.css` and, for the first time, the two INLINE styles in `Hero.tsx` (the `▸` glyph and the `TRY:` label). `--faint` is now consumed by nothing and is kept declared with the measurement written beside it | #396 — the issue was filed against `.composer-hint` alone (11.5px, 2.77:1 light / 3.49:1 dark) and asked for a sweep. The sweep overturned its own plan: `--faint`'s BEST light-theme ratio against any surface it paints on is **2.92:1** (on `--surface`), under even the relaxed **3:1 large-text** floor, so the one rule the plan meant to KEEP on the token — `.step-number`, 24px/700 — had to move too. Measured in Chromium: it paints on the card's `--surface`, not on the inset panel's `--surface-2`, because `.step-meta` is a SIBLING of `.step-preview`. An inline style beats every rule, so the two `Hero.tsx` uses were invisible to every CSS-only sweep including the issue's own. **A second, worse finding came out of the same sweep and is fixed here: `.cta-banner`'s body `<p>` had no rule at all**, so it inherited the banner's `color: var(--ink)` onto the banner's `background: var(--ink)` — measured 1.00:1 in BOTH themes, a whole sentence invisible to every sighted visitor. `.cta-banner > p:not(.cta-footnote) { color: var(--bg) }` takes it to 16.35:1 light / 15.69:1 dark. **Adversarial review then found two TOTAL bypasses of the new guard, both measured green on every gate before they were closed:** `-webkit-text-fill-color: var(--faint)` (it overrides `color` for the glyph fill while `getComputedStyle().color` keeps reporting the old value — 5.27:1 read, 2.77:1 painted), and `VAR(--faint)` uppercase inside `@media (max-width: 600px)` (two independent causes — a case-sensitive `var` match, and a browser sweep that only ever ran at a desktop viewport). Both halves now sweep `color` AND `-webkit-text-fill-color`, `var` is matched case-insensitively while `--faint` stays case-sensitive, and the browser sweep runs a second pass at 390x844. A skeptic then found a THIRD bypass inside the fix for the second: `[vV][aA][rR]` still anchored on the SPELLING of `var`, and a CSS identifier ESCAPE (`\76 ar(--faint)`, `v\61 r(--faint)`, `var(/*c*/--faint)` — all measured rendering `#9a978f`) slipped past it, planted inside `@media (prefers-reduced-motion: reduce)` where the browser sweep never looks. The rule now matches the TOKEN rather than the function name, so every spelling is covered at once. `.cta-footnote` was also below AA and had been exempted by name without measurement (4.99:1 light, **3.30:1 dark** at 50% alpha); it is now 65%, measured 7.54:1 / 5.34:1. `frontend/scripts/mutate-faint-guards.sh` carries 74 mutants across five suites; the run recorded no survivor, and the list is not claimed to be exhaustive. Of the 22 visual baselines, 18 were regenerated for the `--faint` move (measured in that round, inherited here) and `cta-light`/`cta-dark` for the CTA sentence (re-derived on the final tree at per-pixel `threshold: 0`: 1,001 px light / 1,035 px dark, ratio 0.01 — inside the committed 0.02 tolerance, so they PASSED unregenerated too); `ticker-light`/`ticker-dark` are the only two left untouched |
| 2026-09-12 | The four semantic colours are THEME-SPLIT, `--refusal-amber` became an alias of `--color-warning`, `#hero-input::placeholder` gained a rule, and the two `.notice-*` badges moved off hardcoded `rgba()` onto tokens | #403 — the #396 guard asks "does anything render `--faint`?", which is a question about one token, so failures using OTHER tokens were invisible to it by construction. Nine selectors were below the WCAG 1.4.3 AA floor — ten DOM runs, eleven failing selector x theme measurements, twelve failing DOM-run x theme measurements (`.check-icon` renders twice, which is the whole reason a bare count misleads here); the executable list is `RETUNED` in `frontend/tests/contrast-floor.spec.ts` and no bare count is authoritative. All reproduced from the RENDERED page before anything changed (light / dark): `.refusal-badge` 3.20 / 3.24, `.hero-nudge` 3.68 / pass, `.compare-footer.good` 3.60 / pass, `.check-icon` 3.42 / pass in two instances, `.compare-footer.bad` pass / 3.22, `.source-badge.zero` pass / 2.92, `#hero-input::placeholder` pass / 3.61, `.notice-rate-limit` 4.31 / 3.17, `.notice-error` pass / 2.67. `--color-error` at 13-14px computes 4.29 / 4.07 / 3.79 light and 3.88 / 4.22 / 3.51 dark on `--surface` / `--bg` / `--surface-2`; those two uses are on signed-in surfaces the demo suite cannot mount, so that one is computed rather than rendered. One value per role cannot clear 4.5:1 on both `#ffffff` and `#1e1e21`, so each split in two. `#hero-input::placeholder` was UNSTYLED and rendered in Chromium's built-in `rgb(117, 117, 117)` — the SAME grey in both themes, because the UA default does not follow the page's inverted surfaces. After: 4.72-8.28:1, every run, both themes, measured by `frontend/tests/contrast-floor.spec.ts` rather than restated here. **`tokens.css` declares dark TWICE** and the `@media (prefers-color-scheme: dark)` copy — the one the DEFAULT visitor gets, since nothing writes `data-theme` until the toggle is clicked — carried NO semantic colours at all, so the split alone would have given that reader the light values; `frontend/src/test/tokenThemeParity.test.ts` now fails the build when the two blocks disagree, for `tokens.css` and for `frontend/public/about.css`, which carries the identical two-block shape. **Review finding folded in:** the first draft put `--color-error` and `--color-error-strong` at dE2000 2.61 in light, below the 5.45 they had before and barely past just-noticeable, which matters because all three reds co-occur in one comparison card; they are now 7.59 light / 8.24 dark. **A second finding from the same review, fixed here:** the sweep declares the sticky header unmeasurable (its 12px `backdrop-filter` blur is genuinely not a flat colour), which exempted every text run inside `<header>` — a reviewer painted the logo `#fbfbfb` on the near-white bar and the suite stayed green. The header's fill was `color-mix(in srgb, var(--bg) 82%, transparent)`, and `.cta-banner` scrolls under it, so the nav links measured 3.64:1 light / 4.20:1 dark over `0.82 x --bg + 0.18 x --ink`. It is now 94% — worst case 4.69:1 / 6.08:1 across all five surfaces the page can put behind the bar — and `contrast-floor.spec.ts` composites that exact alpha over each of them rather than trusting the sweep. **A third finding from review, corrected here rather than restated:** this row previously said `comparison-dark` was the only baseline whose pixels move and that the other 21 were byte-identical at per-pixel threshold 0. Both halves are false, and the correction is MEASURED — two consecutive full regenerations on darwin are byte-identical across all 22, so the renderer is deterministic here and every difference below is this change's, not noise. Ten of the 22 move against `origin/main`: `comparison-dark` 29,882 px and `comparison-light` 27,523 px, then `hero` (1,443 light / 1,488 dark), `faq` (1,483 / 941), `how-it-works` (1,319 / 982) and `pricing` (1,400 / 881). The other twelve — `chat-empty`, `cta`, `demo`, `footer`, `personas`, `ticker`, both themes each — really are byte-identical. Only `comparison-dark` is regenerated here; the other nine still hold pre-PR pixels, and **all 22 pass either way**, because the suite sets no `threshold`, so Playwright's default 0.2 ignores a per-channel delta this small (the nine peak at 47 of 255, most far lower) and each is counted as zero differing pixels — well inside the `maxDiffPixelRatio: 0.02` the suite does set. Measured, not inferred: restoring `origin/main`'s `comparison-dark` and running the suite passes too, so even that regeneration was a choice rather than a tolerance failure. None of this is gated anywhere — the visual suite runs in no CI job ([#325](https://github.com/imrohitagrawal/citevyn/issues/325)), which is why a wrong claim about it could stand |
