...
- Mono `.demo-badge`: "LIVE — answering from the docs" in live mode,
  ...
  until 2026-09-08; no such string has ever existed in `ChatView.tsx`.
  Pre-existing spec drift, found while fixing #380 and corrected in the same
  change.)
...
- Disclaimer `.composer-hint`: `JetBrains Mono` 11px, two sentences. The first
  is always "CiteVyn answers from the official docs." The second states the
  MODE, and is keyed on **both** `live` and `chatEmpty` (#380) — because the
  hint renders outside the empty/non-empty branch and so is on screen before any
  answer exists:
  - empty + live → "Answers here are generated live."
  - empty + demo → "Answers here are samples for the demo."
  - non-empty + live → "This answer was generated live, just now."
  - non-empty + demo → "This is a sample answer for the demo."

  The empty-state pair is deliberately GENERAL ("Answers here…") rather than
  about a specific answer, so it claims nothing about content that is not on
  screen. It is not dropped when empty: a mode sentence keeps the live/demo
  signal at the moment the reader is about to type, and it lets the empty-state
  test assert a string that is PRESENT rather than only assert an absence.
...
| 2026-09-08 | The composer hint's second sentence is keyed on `chatEmpty` as well as `live`: an empty chat now reads "Answers here are generated live." / "Answers here are samples for the demo." instead of "This answer was generated live, just now." Two stale spec lines in §3 corrected in passing | #380 — `.composer-hint` renders OUTSIDE the `chatEmpty ? … : …` branch (which opens at `ChatView.tsx:478` and closes at `:577`, while the composer starts at `:606` and the hint at `:675`), so the sentence was on screen before any answer existed and asserted one did. `chatEmpty` and `pending` CAN be true together — a request in flight over a still-empty transcript, pinned by an existing test — so the hint read "This answer was generated live, just now." while the loader above it said "Searching the docs…". **Deliberately NOT the issue's suggested fix**, which was to drop the sentence when empty: a mode-stating sentence keeps the live/demo signal at the moment the reader is about to act, and it lets the empty-state test assert a string that is PRESENT rather than only assert an absence — a non-vacuous guard. The empty-state strings are GENERAL ("Answers here…") rather than about a specific answer precisely so they claim nothing about content that is not on screen. No test asserted this copy before and `live` was `true` in ZERO vitest tests; there are now four, each asserting the full rendered `textContent` with `toBe`, and 6 mutants added to `frontend/scripts/mutate-a11y-guards.sh`. Two §3 bullets were found stale while writing this and fixed in the same change: both quoted "DEMO — canned responses", a string that does not exist in `ChatView.tsx`; the top bar renders "LIVE — answering from the docs" / "DEMO — sample answers". **No behavior change beyond the copy.** **+29 B gzip, measured** (62,974 -> 63,003; budget 64,479, headroom 1,505 -> 1,476). **Both `chat-empty` visual baselines pass UNRECORDED** — one changed line of 11.5px text stays inside `maxDiffPixelRatio: 0.02` (2 passed). That pass is not vacuous: replacing the empty-state demo string with a deliberately long three-line one FAILS both baselines at ratio 0.04 (19,003 px light / 18,220 px dark), so the gate does see this element. `visual.spec.ts` is `testIgnore`d by `playwright.demo-ci.config.ts`, so that was a local check, not a CI one |
