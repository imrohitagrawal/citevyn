...
- **Italic accents:** a serif italic, rendered by the reader's `serif` fallback.
  ...
  `git log -S"Newsreader" -- index.html src/styles/ public/` finds no commit that
  ever linked it, and the only mention in the code is `landing.css:801`'s
  `font-family: "Newsreader", serif`, whose first entry has always been
  unavailable. Checked while fixing #365 rather than assumed; the doc was
