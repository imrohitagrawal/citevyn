...
| 2026-09-06 | A keyboard-focused ticker chip is raised above the marquee's edge fades; the focus sweep now reads `::before`/`::after` | Skeptic round on the #355 fix. `backdropOf` composited `getComputedStyle(n)` only, so pseudo-elements were invisible to it — and the occluder that ACTUALLY SHIPS is one: `.ticker-strip::before/::after` are 80 px `linear-gradient(to right, var(--bg), transparent)` fades at `z-index: 1`, while `.ticker-chip` is a plain non-positioned `<button>` in the tab order, so the fade paints OVER its focus ring. Measured on a NATURAL Tab walk in light theme, nothing forced: **ring contrast 1.85:1 at the first ticker stop**, and **1.47:1** pixel-sampled from a screenshot deeper into the fade (painted ring `rgb(210,209,206)` on `rgb(251,251,248)`) — up to 2× below the 3:1 floor — while the guard reported a confident **17.21:1**. Fixing only the product would leave the guard lying for the next overlay; fixing only the guard would turn it red without making the ring visible, so both halves landed together. `.ticker-chip:focus-visible { position: relative; z-index: 2 }` touches the focus state only, so there is no layout shift, no stacking context at rest, and all 22 visual baselines pass unregenerated. `backdropOf` now reads both pseudo-elements and refuses to score a ring under a painting one **unless the control is raised above it** — which is what stops the check false-positiving on every ticker chip once the CSS fix is in. Proven in both directions: with the CSS reverted the sweep names all eight ticker chips; with it applied the painted ring pixel-samples at **8.96:1** |
