...
| 2026-09-05 | Both drawers now trap Tab, via a shared `useFocusTrap` hook extracted from `AuthModal` | #331 — `HistoryDrawer` and `ConnectedAccountsDrawer` set `aria-modal="true"` and put a backdrop over the page, so the mouse could not reach the controls behind them, but the keyboard could: measured in real Chromium, **3 forward Tabs reached `BODY` and then the page nav, and ONE Shift+Tab landed on a page button**. Both files had documented the omission as *"no form fields, so no Tab trap"* — but the hazard is escaping the dialog, not moving between inputs. Re-measured after the fix in the same browser: focus cycles inside indefinitely and never leaves. The drawer stands its trap down while the password modal is open on top of it, so the modal's own trap governs and the two do not fight. **No visual change** — keyboard behaviour only |
